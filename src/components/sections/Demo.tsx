...
export default function Demo() {
  ...
  return (
    <section
      ref={sectionRef}
      id="demo"
      className={`relative w-full overflow-hidden bg-[#FAFAFA] ${isMobile ? 'py-24' : ''}`}
      style={isMobile ? {} : { height: '100vh' }}
    >
      <div className={`${isMobile ? '' : 'sticky top-0 h-screen'} w-full`}>
        <div className="px-6 pt-16 md:pt-20">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="mt-2 max-w-2xl text-[#666666]">{t('demo.sectionSubtitle')}</p>
          </div>
        </div>

        <div
          ref={trackRef}
          className={`mt-8 flex gap-6 px-6 md:mt-12 ${isMobile ? 'flex-col' : 'h-[calc(100vh-220px)] items-center'}`}
        >
          {/* Card 1: TOEFL Demo */}
          <div
            className={`flex-shrink-0 rounded-2xl border border-[#E5E5E5] bg-white p-4 md:p-6 ${
              isMobile ? 'w-full' : 'w-[70vw] max-w-4xl'
            }`}
          >
          ...
          {/* Card 2: Coming Soon */}
          <div
            className={`flex flex-shrink-0 flex-col justify-center rounded-2xl border border-[#E5E5E5] bg-[#F3F4F6] p-8 text-center ${
              isMobile ? 'w-full py-16' : 'w-[50vw] max-w-md'
            }`}
          >
          ...
          {/* Card 3: CTA */}
          <div
            className={`flex flex-shrink-0 flex-col justify-center rounded-2xl bg-gradient-to-br from-[#2563EB] to-[#1D4ED8] p-8 text-white md:p-10 ${
              isMobile ? 'w-full py-16' : 'w-[50vw] max-w-md'
            }`}
          >
            ...
            <button
              ...
              className="mt-6 inline-block self-start rounded-full bg-white px-6 py-2 text-sm font-semibold text-[#2563EB] transition-transform hover:scale-105"
            >
              {t('demo.card3Cta')}
            </button>
          </div>
